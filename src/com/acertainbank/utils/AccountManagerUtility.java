...
import com.acertainbank.client.AccountManagerClientConstants;
...
public final class AccountManagerUtility {
	 ...
	 * @return
	 */
	public static AccountManagerMessageTag convertURItoMessageTag(
			String requestURI) {
	 ...
	 */
	public static JSONObject SendAndRecv(HttpClient client,
		...
		try {
			...
			throw new AccountManagerException(
					AccountManagerClientConstants.strERR_CLIENT_REQUEST_SENDING,
					ex);
		}
		...
		if (exchangeState == HttpExchange.STATUS_COMPLETED) {
			try {
				AccountManagerResponse accountManagerResponse = (AccountManagerResponse) AccountManagerUtility
						.deserializeXMLStringToObject(exchange
								.getResponseContent().trim());
				AccountManagerException ex = accountManagerResponse
						.getException();
				if (ex != null) {
					throw ex;
				}
				return accountManagerResponse.getResult();

			} catch (UnsupportedEncodingException ex) {
				throw new AccountManagerException(
						AccountManagerClientConstants.strERR_CLIENT_RESPONSE_DECODING,
						ex);
			}
		} else if (exchangeState == HttpExchange.STATUS_EXCEPTED) {
			throw new AccountManagerException(
					AccountManagerClientConstants.strERR_CLIENT_REQUEST_EXCEPTION);
		} else if (exchangeState == HttpExchange.STATUS_EXPIRED) {
			throw new AccountManagerException(
					AccountManagerClientConstants.strERR_CLIENT_REQUEST_TIMEOUT);
		} else {
			throw new AccountManagerException(
					AccountManagerClientConstants.strERR_CLIENT_UNKNOWN);
		}
	}
